...
export function getRecordBoard(board) {
	...
	onValue(UserstRef, (snapshot) => {
		...
		sortedUsers.forEach(el => {
			...
			if (numScore <= 10) {
				if (el[3] === localStorage.username) { userScoreOutline = 'style="text-shadow: #fff 1px 0 10px;"' }
				userRecord.innerHTML = `<div ${userScoreOutline} class="user-score"><span class="score-username">${numScore}. ${el[3]}: </span><span class="score-score"> ${el[2]}╭ᑎ╮${el[0]}</span></div>`
				userScoreOutline = ''
